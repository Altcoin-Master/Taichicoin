...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/TaichicoinParams"
else
    PARAMS_DIR="$HOME/.taichicoin-params"
fi
...
DOWNLOAD_URL="https://github.com/Altcoin-Master/taichicoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with TCC_DISABLE_SOMETHING=1
TCC_DISABLE_WGET="${TCC_DISABLE_WGET:-}"
#TCC_DISABLE_IPFS="${TCC_DISABLE_IPFS:-}"
TCC_DISABLE_CURL="${TCC_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$TCC_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$TCC_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$TCC_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Taichicoin parameters!
...
This script will fetch the Taichicoin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Taichicoin SNARK parameters. Note that it is
