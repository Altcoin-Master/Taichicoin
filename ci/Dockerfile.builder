...
# taichicoin_hash
RUN git clone https://github.com/Altcoin-Master/taichicoin_hash
RUN cd taichicoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} taichicoin
RUN useradd -u ${USER_ID} -g taichicoin -s /bin/bash -m -d /taichicoin taichicoin
...
RUN mkdir /taichicoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /taichicoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /taichicoin-src
