...
const char * const BITCOIN_CONF_FILENAME = "taichicoin.conf";
const char * const BITCOIN_PID_FILENAME = "taichicoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "taichicoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TaichicoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TaichicoinCore
    // Mac: ~/Library/Application Support/TaichicoinCore
    // Unix: ~/.taichicoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TaichicoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TaichicoinCore";
#else
    // Unix
    return pathRet / ".taichicoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty taichicoin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Taichicoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Taichicoin Core developers";
    }
